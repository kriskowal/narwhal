...
function createRecurrenceIterator(rule, dtStart, timezone) {
    assert.isTrue('function' === typeof timezone);
    assert.isTrue('number' === typeof dtStart);
    assert.isTrue('function' === typeof rule.getAttribute);

    function soleValue(name) {
        var values = rule.getAttribute(name);
        assert.isTrue(values === null || values instanceof Array);
        return values && values[0].toUpperCase();
    }

    function apply(array, xform) {
        assert.isTrue(typeof xform === 'function');
        var out = [];
        if (array !== null) {
...
exports.createRecurrenceIterator = createRecurrenceIterator;
