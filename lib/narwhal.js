...
parser.option('-d', '--debug', 'debug')
    .help('set debug mode, ENGINE.debug = true')
    .inc();
...
parser.option('-V', '--version')
    ...
    .action(function () {
        var packageJson = FS.path(ENGINE.prefix).join("package.json").read({"charset": "UTF-8"});
        var version = JSON.decode(packageJson).version;
        this.print(
            "narwhal " +
            version +
            " " +
...
if (ENGINE.setOptimizationLevel) {
...
exports.ensureEngine = function(engines) {
    ...
    SYSTEM.stderr.print('Notice: '+FS.base(SYSTEM.args[0])+' is incompatible with "'+SYSTEM.engine+'" engine. '+
