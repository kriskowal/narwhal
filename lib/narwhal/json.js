...
parser.option('-j', '-}', '--json-in', 'jsonIn')
    ...
    .action(function (options) {
        options._ = block(options._, options.mode, function (_) {
            if (!typeof _ == "string") {
                SYSTEM.stderr.print("JSON can only parse strings.");
                OS.exit(-1);
            }
...
parser.option('-J', '-{', '--json-out', 'jsonOut')
    .help('json.stringify(_) -> _')
    .action(function (options) {
        options._ = collect(options._);
        options._ = block(options._, options.mode, function (_) {
            _ = collect(_);
            return JSON.stringify(_, null, options.tabWidth);
        });
    });
...
parser.option('-e', '--evaluate', 'expression')
    ...
    .action(function (options, name, expression) {
        options._ = block(options._, options.mode, function (_) {
            global._ = _;
            return ENGINE.compile(expression, '-e');
        });
    });
...
parser.option('-v', '--value', 'expression')
    ...
    .action(function (options, name, expression) {
        options._ = block(options._, options.mode, function (_) {
            global._ = _;
            return ENGINE.compile('(' + expression + ')', '-v');
        });
    });
...
parser.option('-w', '--where', 'expression')
    ...
    .action(function (options, name, expression) {
        ...
        options._.forEach(function (_) {
            global._ = _;
            if (ENGINE.compile(expression, '-w'))
                results.push(_);
        });
        options._ = results;
    })
