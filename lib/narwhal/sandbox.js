 ...
 *    loader, which can be used in nested sandboxes.
 * * `require.force(id)`
...
exports.Sandbox = function (options) {
    ...
    var sandbox = function (id, baseId, pkg, basePkg, options) {
        ...
        if (!Object.prototype.hasOwnProperty.call(exportsMemo, id) || options.force || options.once) {
            ...
            var completed;
            try {
                var returnedExports = factory(scope);
                if (returnedExports) {
                    if (graph[id]) {
                        throw new Error('Module "' + id + '" returned an ' +
                        "alternate exports object, but its exports have " +
                        "already been received by cyclic dependencies " + 
                        graph[id].map(function (id) {
                            return '"' + id + '"';
                        }).join(', '));
                    } else {
    ...
    */
    ...
    /***
     * @param id
     * @param baseId (optional if `id` is top-level)
     * @param pkg (optional)
     * @param basePkg (optional)
     */
...
/**
 * Creates a sandbox and executes a main module in the
 * sandbox.
 * @param {String} main a top-level identifier for the main
 * module to execute in the sandbox.
 * @param {{print, ...}} system an alternate system module.
 * @param {{prefix, loader, modules, print, debug}} options
 */
