...
var rrule = require('calendar/rrule');
...
function runRecurrenceIteratorTest(
    ...
    if (limit < 0) { sb.push("..."); }
    assert.eq(golden, sb.join(','));
    console.groupEnd();
}

register('testFrequencyLimits', function testFrequencyLimits() {
    try {
...
// from section 4.3.10
register('testMultipleByParts', function testMultipleByParts() {
    runRecurrenceIteratorTest(
...
register('testCountWithInterval', function testCountWithInterval() {
    runRecurrenceIteratorTest(
...
// from section 4.6.5
register('testNegativeOffsets', function testNegativeOffsets() {
    runRecurrenceIteratorTest(
...
register('testDailyFor10Occ', function testDailyFor10Occ() {
    runRecurrenceIteratorTest(
...
register('testDailyUntilDec4', function testDailyUntilDec4() {
    runRecurrenceIteratorTest(
...
register('testEveryOtherDayForever', function testEveryOtherDayForever() {
    runRecurrenceIteratorTest(
...
register('testEvery10Days5Occ', function testEvery10Days5Occ() {
    runRecurrenceIteratorTest(
...
register('testWeeklyFor10Occ', function testWeeklyFor10Occ() {
    runRecurrenceIteratorTest(
...
register('testWeeklyUntilDec24', function testWeeklyUntilDec24() {
    runRecurrenceIteratorTest(
...
register('testLastWeekdayOfMonth', function testLastWeekdayOfMonth() {
    runRecurrenceIteratorTest(
...
register(
    ...
    assert.eq(time.parseIcal("20000101"), riter.next());
    assert.eq(time.parseIcal("20000102"), riter.next());
    assert.eq(time.parseIcal("20000103"), riter.next());
});

register('testNoInstancesGenerated', function testNoInstancesGenerated() {
    var riter = rrule.createRecurrenceIterator(
            new StubContentLine("RRULE:FREQ=DAILY;UNTIL=19990101"),
            time.parseIcal("20000101"), timezone.utc);
    assert.isTrue(!riter.hasNext());

    assert.isNull(riter.next());
    assert.isNull(riter.next());
    assert.isNull(riter.next());
});
...
register('testLastWeekdayOfMarch', function testLastWeekdayOfMarch() {
    runRecurrenceIteratorTest(
...
register('testFirstWeekdayOfMarch', function testFirstWeekdayOfMarch() {
    runRecurrenceIteratorTest(
            "RRULE:FREQ=MONTHLY;BYMONTH=3;BYDAY=SA,SU;BYSETPOS=1",
            time.parseIcal("20000101"), 4,
...
register('testFirstWeekdayOfYear', function testFirstWeekdayOfYear() {
    runRecurrenceIteratorTest(
...
register('testSecondWeekday1', function testSecondWeekday1() {
    runRecurrenceIteratorTest(
...
register('testSecondWeekday2', function testSecondWeekday2() {
    runRecurrenceIteratorTest(
            "RRULE:FREQ=WEEKLY;BYDAY=MO,TU,WE,TH,FR;BYSETPOS=2",
...
register('testLastWeekOfTheYear', function testLastWeekOfTheYear() {
    runRecurrenceIteratorTest(
...
register('testUserSubmittedTest1', function testUserSubmittedTest1() {
    runRecurrenceIteratorTest(
...
register('testAdvanceTo', function testAdvanceTo() {
...
register('testCaseThatYieldedDupes', function testCaseThatYieldedDupes() {
    runRecurrenceIteratorTest(
